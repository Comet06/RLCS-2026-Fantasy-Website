import { EventSums, splitSums, major1Rounds, major2Rounds, playerScores, members, teams, prizes, year } from "./a-current-page-data.js";
import { determineRegionalSums } from "./a-regionals.js";
import { determineMajorSums } from "./a-majors.js";
import { determineChampionshipSums } from "./a-championship.js";
...
window.addEventListener('load', function() {
  if (window.location.pathname === `${path}/index.html`) {
    ...
    document.getElementById('scripts').innerHTML = `
      <script src="js/a-major-form.js"></script>
    `
...
function determineTotalScores(){
  members.forEach((id)=>{
    ...
    playerScores[id.shortname] = [EventSums[id.shortname][0], EventSums[id.shortname][1], EventSums[id.shortname][2], major1Rounds[id.shortname][2], splitSums[id.shortname][0], 
                                  EventSums[id.shortname][3], EventSums[id.shortname][4], EventSums[id.shortname][5], major2Rounds[id.shortname][2], splitSums[id.shortname][1], splitSums[id.shortname][2], splitSums[id.shortname][3]];
  })
}
function sortPlayerScores(index){ // 0 = split1, 1 = split2, 2 = championship
  determineRegionalSums()
  determineMajorSums()
  determineChampionshipSums()
  determineTotalScores()
